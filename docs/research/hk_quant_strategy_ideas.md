...
| ---: | --- | --- | --- | --- | --- |
| 1 | `hk_etf_regime_rotation` | `market_history` | Low turnover, liquid ETF universe, avoids single-name borrow and disclosure problems | ETF choice and stamp-duty exemption must be checked per symbol | `research_candidate` |
...
1. Backtest `hk_etf_regime_rotation` next because it reuses `market_history` and is operationally simple.
