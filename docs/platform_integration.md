...
## 2026-06-02 VCM / CAS microstructure shock future gate

Platform integration now mirrors `hk_vcm_cas_microstructure_shock_risk_overlay` as a non-selectable snapshot future-research candidate. Switch-plan tooling must treat it as pre-scaffold only: no runtime profile, no high-frequency or auction-trading broker order surface, and no Cloud Run mutation should be inferred. If promoted later, it must pass the snapshot repository's new-contract requirement plus point-in-time VCM securities list, trigger timestamp, reference price, cooling-off band, POS/CAS reference price, auction status, derivatives VCM coverage, broker order-preview rejection, same-universe ablation, HK cost/capacity evidence, dry-run order previews, bilingual notification logs, rollout controls, and operator approval.

平台集成现在镜像 `hk_vcm_cas_microstructure_shock_risk_overlay` 作为 non-selectable snapshot future-research 候选。Switch-plan tooling 必须把它视为 pre-scaffold：没有 runtime profile、没有高频或竞价交易券商下单入口，也不能从该候选推断任何 Cloud Run 变更。若后续提升，必须先通过 snapshot 仓库的新 contract 要求，并补齐 point-in-time VCM securities list、trigger timestamp、reference price、cooling-off band、POS/CAS reference price、auction status、derivatives VCM coverage、broker order-preview rejection、同 universe ablation、港股成本/容量证据、dry-run order previews、双语通知日志、rollout controls 和 operator approval。

## 2026-06-02 REIT dividend-spread / rate-sensitivity future gate

`hk_reit_dividend_spread_rate_sensitivity_overlay` is a mirrored snapshot future-research gate only. Keep it non-selectable on all platforms until a new snapshot contract, point-in-time REIT universe/distribution/NAV/GAV/gearing/rate-spread evidence, same-universe ablation, <=30% drawdown walk-forward evidence, dry-run order previews, bilingual notifications, rollout controls, and operator approval exist. Do not expose a leveraged REIT carry, margin, property-developer proxy, or dividend-capture order surface.

`hk_reit_dividend_spread_rate_sensitivity_overlay` 只是镜像 snapshot future-research gate。所有平台都必须保持 non-selectable，直到新建 snapshot contract，并补齐点时间 REIT universe / distribution / NAV / GAV / gearing / rate-spread 证据、同 universe ablation、最大回撤 <=30% 的 walk-forward 证据、dry-run order preview、双语通知、rollout controls 和 operator approval。不要暴露 leveraged REIT carry、margin、property-developer proxy 或 dividend-capture 下单入口。

## 2026-06-02 regulatory enforcement / disciplinary risk future gate

`hk_regulatory_enforcement_disciplinary_risk_overlay` is a mirrored snapshot future-research gate only. Keep it non-selectable on all platforms until a new snapshot contract, timestamped HKEX/SFC enforcement records, issuer/director mapping, sanction severity, halt/suspension/resumption/liquidity context, same-universe governance ablation, <=30% drawdown walk-forward evidence, dry-run order previews, bilingual notifications, rollout controls, and operator approval exist. Do not expose a rumour-trading, event-chasing, litigation-arbitrage, or short-selling order surface.

`hk_regulatory_enforcement_disciplinary_risk_overlay` 只是镜像 snapshot future-research gate。所有平台都必须保持 non-selectable，直到新建 snapshot contract，并补齐 timestamped HKEX/SFC enforcement records、issuer/director mapping、sanction severity、halt/suspension/resumption/liquidity context、same-universe governance ablation、最大回撤 <=30% 的 walk-forward 证据、dry-run order preview、双语通知、rollout controls 和 operator approval。不要暴露 rumour-trading、event-chasing、litigation-arbitrage 或 short-selling 下单入口。

## 2026-06-02 margin financing / collateral forced-selling risk future gate

`hk_margin_financing_collateral_forced_selling_risk_overlay` is a mirrored snapshot future-research gate only. Keep it non-selectable on all platforms until a new snapshot contract, point-in-time marginable-security / haircut / LTV / collateral-concentration evidence, SFC SMF guideline and review provenance, share-pledge / forced-sale disclosure, halt/suspension/resumption/liquidity context, same-universe ablation, <=30% drawdown walk-forward evidence, dry-run order previews, bilingual notifications, rollout controls, and operator approval exist. Do not expose a leveraged margin-trading, forced-liquidation front-running, rumour-trading, or short-selling order surface.

`hk_margin_financing_collateral_forced_selling_risk_overlay` 只是镜像 snapshot future-research gate。所有平台都必须保持 non-selectable，直到新建 snapshot contract，并补齐 point-in-time marginable-security / haircut / LTV / collateral-concentration evidence、SFC SMF guideline and review provenance、share-pledge / forced-sale disclosure、halt/suspension/resumption/liquidity context、same-universe ablation、最大回撤 <=30% 的 walk-forward 证据、dry-run order preview、双语通知、rollout controls 和 operator approval。不要暴露 leveraged margin-trading、forced-liquidation front-running、rumour-trading 或 short-selling 下单入口。

## 2026-06-02 liquid large-cap weekly reversal future gate

`hk_liquid_largecap_weekly_reversal_cost_aware_overlay` is a mirrored snapshot future-research gate only. Keep it non-selectable on all platforms until a new snapshot contract, point-in-time weekly-return / liquidity / cost / VCM-CAS evidence, same-universe momentum/liquidity/volatility/event ablation, <=30% drawdown walk-forward evidence, dry-run order previews, bilingual notifications, rollout controls, and operator approval exist. Do not expose an intraday, high-turnover, microcap, opening-gap, or pure contrarian order surface.

`hk_liquid_largecap_weekly_reversal_cost_aware_overlay` 只是镜像 snapshot future-research gate。所有平台都必须保持 non-selectable，直到新建 snapshot contract，并补齐 point-in-time weekly-return / liquidity / cost / VCM-CAS evidence、same-universe momentum / liquidity / volatility / event ablation、最大回撤 <=30% 的 walk-forward 证据、dry-run order preview、双语通知、rollout controls 和 operator approval。不要暴露 intraday、高换手、microcap、opening-gap 或纯 contrarian 下单入口。


## 2026-06-02 US ADR / HK secondary-listing lead-lag future gate

`hk_us_adr_hk_secondary_listing_lead_lag_overlay` is a mirrored snapshot future-research gate only. Keep it non-selectable on all platforms until a new snapshot contract, point-in-time ADR/HK mapping, ADR ratio, primary/secondary listing status, issuer information sheet, adjusted US/HK close, after-hours/pre-market context, USD/HKD FX, corporate-action, conversion/fungibility, holiday/delisting, broker-permission, same-universe ablation, <=30% drawdown walk-forward evidence, dry-run order previews, bilingual notifications, rollout controls, and operator approval exist. Do not expose cross-market arbitrage, conversion, pair-trading, short-selling, margin or after-hours order surfaces.

`hk_us_adr_hk_secondary_listing_lead_lag_overlay` 只是镜像 snapshot future-research gate。所有平台都必须保持 non-selectable，直到新建 snapshot contract，并补齐 point-in-time ADR/HK mapping、ADR ratio、primary/secondary listing status、issuer information sheet、adjusted US/HK close、after-hours/pre-market context、USD/HKD FX、corporate-action、conversion/fungibility、holiday/delisting、broker-permission、同 universe ablation、最大回撤 <=30% 的 walk-forward 证据、dry-run order preview、双语通知、rollout controls 和 operator approval。不要暴露 cross-market arbitrage、conversion、pair-trading、short-selling、margin 或 after-hours 下单入口。


## 2026-06-02 smart beta factor-regime rotation future gate

`hk_smart_beta_factor_regime_rotation_overlay` is a mirrored snapshot future-research gate only. Keep it non-selectable on all platforms until a new snapshot contract, point-in-time factor-index methodology, constituents, scores, factor returns, rebalance buffers, weight caps, market-cycle / volatility / sentiment / macro-state labels, factor-correlation, same-universe ablation, <=30% drawdown walk-forward evidence, dry-run order previews, bilingual notifications, rollout controls, and operator approval exist. Do not expose black-box market timing, high-turnover factor chasing, leveraged, short, derivative or intraday order surfaces.

`hk_smart_beta_factor_regime_rotation_overlay` 只是镜像 snapshot future-research gate。所有平台都必须保持 non-selectable，直到新建 snapshot contract，并补齐 point-in-time factor-index methodology、constituents、scores、factor returns、rebalance buffers、weight caps、market-cycle / volatility / sentiment / macro-state labels、factor-correlation、同 universe ablation、最大回撤 <=30% 的 walk-forward 证据、dry-run order preview、双语通知、rollout controls 和 operator approval。不要暴露黑箱 market timing、高换手 factor chasing、杠杆、做空、衍生品或 intraday 下单入口。
