...
STATUS_ICON = "🇭🇰"
PROFILE_NAME = "hk_blue_chip_leader_rotation"
BENCHMARK_SYMBOL = "02800"
BROAD_BENCHMARK_SYMBOL = "02800"
SAFE_HAVEN = "02800"
DEFAULT_DYNAMIC_UNIVERSE_SIZE = 30
DEFAULT_HOLDINGS_COUNT = 6
DEFAULT_SINGLE_NAME_CAP = 0.18
DEFAULT_MIN_POSITION_VALUE_HKD = 20_000.0
DEFAULT_HOLD_BUFFER = 2
DEFAULT_HOLD_BONUS = 0.08
...
SNAPSHOT_CONTRACT_VERSION = "hk_blue_chip_leader_rotation.feature_snapshot.v1"
...
def _to_frame(feature_snapshot: Any) -> pd.DataFrame:
    ...
    for optional_numeric in ("market_cap_hkd", "lot_size"):
        if optional_numeric in frame.columns:
            frame[optional_numeric] = pd.to_numeric(frame[optional_numeric], errors="coerce")
    return frame
...
def _resolve_effective_holdings_count(
    *,
    ...
    if stock_exposure <= 0:
        return 0
    if portfolio_total_equity is None or min_position_value_hkd <= 0:
        return requested_count

    target_stock_value = float(portfolio_total_equity) * float(stock_exposure)
    if target_stock_value <= 0:
        return 0
    count_by_value = max(1, int(math.floor(target_stock_value / float(min_position_value_hkd))))
...
def build_target_weights(
    ...
    *,
    ...
    min_position_value_hkd: float = DEFAULT_MIN_POSITION_VALUE_HKD,
    ...
    broad_benchmark_symbol = normalize_symbol(broad_benchmark_symbol)
    ...
    effective_holdings_count = _resolve_effective_holdings_count(
        ...
        min_position_value_hkd=float(min_position_value_hkd),
    )
...
def extract_managed_symbols(
    ...
    *,
    ...
    frame = _to_frame(feature_snapshot)
    excluded = {normalize_symbol(benchmark_symbol), normalize_symbol(broad_benchmark_symbol)}
    safe_haven = normalize_symbol(safe_haven)
...
def _load_calendar() -> tuple[Any | None, str]:
    ...
    try:
        calendar = module.get_calendar("XHKG")
    except Exception:
        return None, "business_day_fallback"
    if calendar is None:
        return None, "business_day_fallback"
    return calendar, "xhkg_calendar"
...
def evaluate_execution_window(
    ...
    *,
    ...
    frame = _to_frame(feature_snapshot)
    date_column = next((column for column in SNAPSHOT_DATE_COLUMNS if column in frame.columns), None)
    if date_column is None:
        return {"should_execute": True, "no_op_reason": None, "snapshot_as_of": None, "execution_window": ()}

    snapshot_as_of = pd.Timestamp(frame[date_column].max()).normalize()
...
def compute_signals(
    ...
    **kwargs: Any,
    ...
    if not execution_window["should_execute"]:
        ...
        return (
            None,
            "monthly HK snapshot cadence | waiting inside execution window",
            False,
            status_desc,
            {
    ...
    signal_desc = (
        f"hk regime={metadata['regime']} breadth={metadata['breadth_ratio']:.1%} "
